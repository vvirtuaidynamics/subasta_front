<template>
  ...
  -->
  <q-select ref="refEl" :name="props.name" :label="props.label" v-bind="fieldOptions" :loading="loading"
            :rules="fieldRules" multiple
            :option-label="props.options?.optionLabel ?? 'label'"
            :option-value="props.options?.optionValue ?? props.data_url.length ? 'id': 'value'"
            :option-disable="props.options?.optionDisable||'disable'"
            :options="OptionsFilters" :model-value="selectedModel"
...
onBeforeMount(async () => {
  ...
  if (props.options?.value)
    selectedModel.value = props.options?.value
...
function filterFn(val, update, abort) {
  update(
    (sel) => {
      if (val === '') {
        ...
        OptionsFilters.value = Options.value.filter(
          (v) => v[props.options?.optionLabel || 'label']?.toLowerCase().indexOf(needle) > -1
        );
    ...
    (refEl) => {
      if (val !== '' && refEl.options?.length > 0 && refEl.getOptionIndex() === -1) {
...
watch(() => Options.value, (new_opts) => {
  if (!props.preload && props.data_url.length)
    OptionsFilters.value = [...new_opts]
})

defineExpose({
  refEl, reset, validate, resetValidation, value
})
