export default {
  CxMsg: "The conection with database service was restore!",
  app: {
    name: "auction",
    description: "",
  },
  chart: "Chart",
  conditions: {
    condition: "Condition",
    contains: "Contains",
    distinct: "Distinct of",
    end: "Ends with",
    equal: "Equals to",
    not_contains: "No contains",
    start: "Start with",
  },
  copyright: "Copyright",
  cxBdMsgSuccess: "Connection with database was success!",
  passwordChangeSuccess: "Password changed successful!",
  passwordChangeError: "Password changed failed!",
  register: "Sign Up",
  rememberMe: "Remember me.",

  error400ListMsg:
    "Something went wrong while processing your request. Invalid filter. ${error}",
  error400RemoveMsg: "Error on remove value(s) to field ${field}",
  error400UpdateMsg: "Failed to update record. ${error}",
  error403Msg: "Error: Access denied",
  error404Msg: "Error: The requested resource wasn't found",
  error666Msg: "The licenced key has expired! Get a new one.",
  errorBdMsg: "Error can't connect with database!",
  errorValidation: "Can't submit, the form have validation errors",
  width: "Width",
  height: "Height",

  avatarEditorModalTitle: "Crop or edit image",
  modelLabel: "Model",
  modelPlural: "Models",
  modelAppLabel: "Aplication",
  modelAppPlural: "Aplications",
  modelUsuarioLabel: "User",
  modelUsuarioPlural: "Users",
  modelRolLabel: "Rol",
  modelRolPlural: "Roles",
  modelPermisoLabel: "Permission",
  modelPermisoPlural: "Permissions",
  modelProvinciaLabel: "Provincia",
  modelProvinciaPlural: "Provincias",
  modelMunicipioLabel: "Municipio",
  modelMunicipioPlural: "Municipios",
  login: "Login",
  loginTip: "Click on the button for login",
  home: "Go to home",
  homeTip: "Click on the button to go home",
  darkActivateTip: "Activate dark mode",
  darkDeactivateTip: "Deactivate dark mode",
  fullscreenBtn: "fullscreen mode",
  enable: "Enable",
  disable: "Disable",
  labelUrlImage: "Image URL.",
  labelInsertImage: "Insert image",
  labelInsertUrlImage: "Input the image url.",
  labelGoHome: "Go to home page",
  myaccount: "My account",
  rols: {
    root: "Administrator",
    seller: "Seller",
  },
  date: {
    days: 'Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday'.split('_'),
    daysShort: 'Sun_Mon_Tue_Wed_Thu_Fri_Sat'.split('_'),
    months: 'January_February_March_April_May_June_July_August_September_October_November_December'.split('_'),
    monthsShort: 'Jan_Feb_Mar_Apr_May_Jun_Jul_Aug_Sep_Oct_Nov_Dec'.split('_'),
    firstDayOfWeek: 0, // 0-6, 0 - Sunday, 1 Monday, ...
    format24h: false,
    pluralDay: 'days'
  },
  fields: {
    identity: "Username or Email",
    date: "Date",
    about_me: "about_me",
    action: "Action",
    active: "Active",
    address: "Address",
    avatar: "avatar",
    birthday: "Birthday",
    company: "Company",
    created: "created",
    deleted: "deleted",
    dni: "dni",
    email: "Email",
    first_name: "First name",
    gender: "Gender",
    id: "id",
    industry: "Industry",
    last_name: "Last name",
    name: "Name",
    surname: "Name",
    note: "note",
    password: "password",
    password_confirm: "confirm password",
    password_new: "new password",
    password_old: "previous password",
    permission: "permission",
    permissions: "permissions",
    phone: "Phone",
    photo: "photo",
    profile: "profile",
    role: "role",
    roles: "roles",
    status: "Status",
    updated: "updated",
    username: "Username",
    validated: "Validated",
    created_at: "Date",
    user: "User",
    model: "Model",
    object: "Object",
    message: "Message",
    type: "Type",
    validated_by: "Validated by",
    date_of_birth: "Birth date",

  },
  file: {
    add: "File add",
    change: "File change",
    clear: "Clear all files",
    delete: "File delete",
    exists: "File exists",
    select: "File select",
  },
  labels: {
    accept: "Accept",
    submit: "Submit",
    actions: "Actions",
    add: "Add",
    addFiles: "Add file(s)",
    all: "All",
    apps: "Applications",
    archive: "Archive",
    cancel: "Cancel",
    change: "Change",
    changePassword: "Change password",
    clear: "Clear",
    clearFiles: "Clear all file(s)",
    clearFilter: "Clear filters",
    clearPicture: "Clear for select another picture",
    clearSelected: "Remove selected icon",
    close: "Close",
    column: "Column",
    columns: "Columns",
    configuration: "Configuration",
    configurationCaption: "User configuration",
    confirm: "Confirm",
    copy: "Copy",
    create: "Create",
    cut: "Cut",
    delete: "Delete",
    deleteFile: "Remove file",
    deleteFiles: "Delete file(s)",
    description: "Description",
    done: "Done",
    edit: "Edit",
    exit: "Exit",
    export: "Export",
    fullImageView: "Click for show pictures",
    hide: "Hide",
    history: "History",
    icon: "Icon",
    iconPerPage: "Icons by page",
    iconSelectTitle: "Select a icon",
    iconSet: "Icon set",
    loading: "Loading",
    maximizeWindow: "Maximize",
    menu: "Menu",
    name: "Name",
    noData: "There is no data to show!",
    noFilterData: "There is no data to show from filter!",
    nodata: "",
    notValidFiles: "file(s) did not pass validation constraints",
    options: "Options menu",
    password: "Password",
    permission: "Permission",
    permissionCaption: "Permissions of user",
    permissions: "Permissions",
    phrase: "Phrase",
    readed: "Check as read",
    reload: "Reload",
    reset: "Reset",
    restoreWindow: "Restore",
    retry: "Retry",
    form: "Form",
    forms: "Forms",
    field: "Field",
    fields: "Fields",
    rol: "Rol",
    rolCaption: "Roles of user",
    roles: "Roles",
    rotate: "Rotate",
    save: "Save",
    saveAndAddOther: "Save and add other",
    search: "Search",
    selectPicture: "Select or edit a picture",
    editPicture: "Edit a picture",
    dropPicture: "Drop a picture",
    show: "Show",
    success: "Action was succefull!",
    update: "Update",
    uploaderFiles: "Select file(s)",
    user: "User",
    view: "View",
    noOptions: "No options",
    yes: "Yes",
    no: "No",
    male: "Male",
    female: "Female",
    other: "Other",
    age: "Age",
    crop: "Crop",
    rotateLeft: "Rotate left 90º",
    rotateRight: "Rotate right 90º",

  },
  language: "Language",
  loginErrorMsg: "Login failed. Bad user or password, try again",
  loginMsg: {
    wrongPassword: "Wrong password",
    wrongPasswordOld: "Wrong old password",
  },
  loginSuccess: "Login success! Welcome ${nombre}",
  logout: "Logout",
  logoutMsg: "Logout success! Godbye ${nombre}",
  lostCxMsg: "Lost conection with database api service!",

  // models: {
  //   administration: "Administration",
  //   bearing: "bearing",
  //   bearings: "Bearings",
  //   carrier: "Carrier",
  //   carriers: "Carriers",
  //   client: "Client",
  //   clients: "Clients",
  //   group: "Group",
  //   groups: "Groups",
  //   home: "Home",
  //   person: "Person",
  //   persons: "Persons",
  //   rol: "Rol",
  //   roles: "Roles",
  //   permission: "Permission",
  //   permissions: "Permissions",
  //   form: "Form",
  //   forms: "Forms",
  //   field: "Field",
  //   fields: "Fields",
  //   user: "User",
  //   users: "Users",
  //   history: "History",
  //   validationtasks: "Validation tasks",
  // },
  newsNofifications: "You have an unread notifications",
  notifications: {
    saveWork: "The text have been save to your local storage",
  },
  token401Error: "Invalid token",
  updateSuccessMsg: "Update success! ",
  validations: {
    required: "Value is required",
    email: "Value must be a valid email address",
    lt: "Value must be lower than ${param}",
    gt: "Value must be greater than ${param}",
    max: "Value must have a maximum of ${param} characters",
    min: "Value must have a minimum of ${param} characters",
    size: "Value must have length of ${size} chars",
    numeric: "Value must be numeric",
    integer: "Value must be a integer",
    boolean: "Value must be valid boolean value",
    url: "Value must be a valid URL",
    alpha: "Value must be a valid alpha",
    alphaNum: "Value must be a valid alpha numeric",
    alphaDash: "Value must be a valid alpha dash character",
    confirmed: "Value must be a confirmed",
    date: "Value must be a valid date",
    phone: "Value must be a valid phone number",
    ipv4: "Value must be a valid ip v4 address",
    ipv6: "Value must be a valid ip v6 address",
    mac: "Value must be a valid mac address",
    hex: "Value must be a valid hex string",
    hexa: "Value must be a valid hexa string",
    hexOrHexa: "Value must be a valid hex or hexa string",
    isNaN: "Value is not valid number",
    file: "Value must be a valid file",
    exist: "Already exist on database",
    unique: "Already have a ${field} with this value",
    sameAs: "The value must be equal to the ${otherName} value",
    validateBefore: "Date must be before ${compareDate}",
    validateAfter: "Date must be after ${compareDate}",
  },
  titles: {
    filter: "Filter options",
    search: "Search options",
    history: "History",
    see: "Data",
  },
  welcome: "Welcome ",
  alerts: {
    noChanges: "No modifications have occurred",
  },

};
